...
export function ChatWindow({ threadId }: { threadId: string }) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      // Build history for the API (include the just-added user msg)
      ...
      if (clean) {
        store.addMessage({ threadId, role: "assistant", content: clean });
      }
  ...
  }, [input, pending, streaming, threadId]);
          ...
          {pending.length > 0 && (
            <div className="mb-2 flex flex-wrap gap-2">
              {pending.map((a, i) => (
                <div
                  key={i}
                  className="surface flex items-center gap-2 rounded-lg px-3 py-1.5 text-xs"
                >
...
function MessageBubble({
  ...
  return (
    <div
      className={cn(
        "group mt-6 flex gap-3 animate-fade-in-up",
